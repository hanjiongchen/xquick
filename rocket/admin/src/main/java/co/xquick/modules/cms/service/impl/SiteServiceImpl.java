...
public class SiteServiceImpl extends CrudServiceImpl<SiteDao, SiteEntity, SiteDTO> implements SiteService {
    ...
    @Override
    protected void beforeSaveOrUpdate(SiteDTO dto, int type) {
    ...
    public boolean updateDto(SiteDTO dto) {
        boolean ret = super.updateDto(dto);
        if (ret) {
            SiteEntity existedEntity = getById(dto.getId());
            if (existedEntity != null && !StringUtils.equals(existedEntity.getCode(), dto.getCode())) {
