...
import com.nb6868.xquick.modules.uc.dao.UserDao;
...
public class UserServiceImpl extends CrudServiceImpl<UserDao, UserEntity, UserDTO> implements UserService {
    ...
    public QueryWrapper<UserEntity> getWrapper(String method, Map<String, Object> params) {
        ...
        if (user.getType() > UcConst.UserTypeEnum.SYSADMIN.value()) {
            qw.in(user.getType() > UcConst.UserTypeEnum.SYSADMIN.value(), "uc_user.dept_id", deptService.getSubDeptIdList(user.getDeptId()));
        }
        // 角色
    ...
    public Result<?> login(HttpServletRequest request, LoginRequest login) {
        ...
        if (null == loginConfig) {
            ...
            // 不需要验证码或验证码正确
            if (UcConst.LoginTypeEnum.ADMIN_USER_PWD.value() == login.getType() || UcConst.LoginTypeEnum.APP_USER_PWD.value() == login.getType()) {
                // 帐号密码登录
            ...
            } else if (UcConst.LoginTypeEnum.ADMIN_MOBILE_PWD.value() == login.getType() || UcConst.LoginTypeEnum.APP_MOBILE_PWD.value() == login.getType()) {
                // 手机号密码登录
            ...
            } else if (UcConst.LoginTypeEnum.ADMIN_MOBILE_SMS.value() == login.getType() || UcConst.LoginTypeEnum.APP_MOBILE_SMS.value() == login.getType()) {
                // 手机号验证码登录
            ...
            } else if (UcConst.LoginTypeEnum.APP_APPLE.value() == login.getType()) {
