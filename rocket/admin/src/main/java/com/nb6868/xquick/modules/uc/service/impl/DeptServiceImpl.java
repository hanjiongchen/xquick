...
import com.nb6868.xquick.modules.uc.dao.DeptDao;
import com.nb6868.xquick.modules.uc.dto.DeptDTO;
import com.nb6868.xquick.modules.uc.dto.DeptTreeDTO;
import com.nb6868.xquick.modules.uc.entity.DeptEntity;
...
public class DeptServiceImpl extends CrudServiceImpl<DeptDao, DeptEntity, DeptDTO> implements DeptService {
    ...
    public boolean logicDeleteById(Serializable id) {
        ...
        /*if (getSubDeptIdList(ids[0]).size() > 1) {
            throw new XquickException(ErrorCode.DEPT_SUB_DELETE_ERROR);
        }

        // 判断部门下面是否有用户
        if (sysUserService.getCountByDeptId(id) {
            throw new XquickException(ErrorCode.DEPT_USER_DELETE_ERROR);
        }
