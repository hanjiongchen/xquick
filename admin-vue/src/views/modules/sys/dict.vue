    ...
    <div class="mod-sys__dict">
      <el-form :inline="true" :model="dataForm">
        ...
        <el-form-item v-if="dataForm.pid === '0'">
          <el-input v-model="dataForm.type" :placeholder="$t('base.type')" clearable/>
        </el-form-item>
        <el-form-item v-if="dataForm.pid !== '0'">
          <el-input v-model="dataForm.value" :placeholder="$t('base.value')" clearable/>
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        <el-table-column prop="type" :label="$t('base.type')" header-align="center" align="center">
          <template slot-scope="scope">
            <span v-if="dataForm.pid !== '0'">{{ scope.row.type }}</span>
            <el-link v-else type="primary" @click="childHandle(scope.row)" :underline="false">{{ scope.row.type }}</el-link>
          </template>
        </el-table-column>
        <el-table-column v-if="dataForm.pid !== '0'" prop="value" label="字典值" header-align="center" align="center"/>
  ...
  methods: {
    ...
    addOrUpdateHandle (row = {}) {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.addOrUpdate.dataForm.type = row.type || this.dataForm.type || ''
