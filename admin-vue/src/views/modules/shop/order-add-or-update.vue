            ...
            <el-row v-for="(item, index) in dataForm.items" :key="index" :prop="'item.' + index + '.value'">
                <el-col :span="12">
                    <el-form-item label="商品规格">
                        <el-input v-model="item.showName" placeholder="选择商品规格" readonly>
                            <sku-pick class="small-button" slot="append" :requestCode="'' + index" v-on:onSkuPicked="onSkuPicked"/>
                        </el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="8">
                    <el-form-item label="数量" prop="num" label-width="50px">
                        <el-input-number v-model="item.qty" placeholder="输入数量" :disabled="!item.skuId" controls-position="right" :min="1" :max="item.stock" class="w-percent-100"/>
                    </el-form-item>
                </el-col>
                <el-col :span="4">
...
export default {
  ...
  components: { UserPick, SkuPick, AmapLocPick },
  data () {
    return {
      // 表单模块参数
  ...
  methods: {
    ...
    // 选中sku
    onSkuPicked (result, requestCode) {
      if (result) {
        const index = Number(requestCode)
        this.dataForm.items[index].skuId = result.sku.id
        this.dataForm.items[index].stock = result.sku.stock
        this.dataForm.items[index].price = result.sku.price
        this.dataForm.items[index].skuName = result.sku.name
        this.dataForm.items[index].spuName = result.spu.name
        this.dataForm.items[index].spuId = result.spu.id
        this.dataForm.items[index].showName = result.spu.name + ` ` + result.sku.name
      }
    },
    // 插入item
    addItem (index) {
      // 插入指定位置
      this.dataForm.items.splice(index, 0, {
        spuName: '',
        spuId: '',
        skuId: '',
        skuName: '',
